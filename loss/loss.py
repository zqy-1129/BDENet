...
class DiverseExpertLoss(nn.Module):
    def __init__(self, cls_num_list=None, max_m=0.5, s=30, tau=2, base_loss='fl'):
        super().__init__()
    ...
    def forward(self, output_logits, target, extra_info=None):
        ...
        expert2_logits = expert2_logits + torch.log(self.prior.view(6, 1, 1).expand(-1 ,256, 256) + 1e-9)
        loss += self.base_loss(expert2_logits, target)

        # Inverse Softmax loss for expert 3
        inverse_prior = self.inverse_prior(self.prior)
        expert3_logits = (expert3_logits + torch.log(self.prior.view(6, 1, 1).expand(-1 ,256, 256) + 1e-9)
                          - self.tau * torch.log(inverse_prior.view(6, 1, 1).expand(-1 ,256, 256) + 1e-9))
