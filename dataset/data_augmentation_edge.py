...
class DataAugmentation:
    ...
    def transform(self, imgs, edges, M_edges, labels, labels_A, labels_B, to_tensor=True):
        """
        :param to_tensor: bool
        :param imgs: [ndarray,]
        :param edges: [ndarray,]
        :param M_edges: [ndarry,]
        :param labels: [ndarray,]
        :param labels_A: [ndarray,]
        :param labels_B: [ndarray,]
        :return: [ndarray,],[ndarray,],[ndarray,],[ndarray,],[ndarray],[ndarray]
        """
        # resize image and convert to tensor
        imgs = [TF.to_pil_image(img) for img in imgs]
        edges = [TF.to_pil_image(img) for img in edges]
        M_edges = [TF.to_pil_image(img) for img in M_edges]
        ...
        if self.with_random_hflip and random.random() > 0.5:
            ...
            edges = [TF.hflip(img) for img in edges]
            M_edges = [TF.hflip(img) for img in M_edges]
        ...
        if self.with_random_vflip and random.random() > 0.5:
            ...
            edges = [TF.vflip(img) for img in edges]
            M_edges = [TF.vflip(img) for img in M_edges]
        ...
        if self.with_random_rot and random.random() > random_base:
            ...
            edges = [TF.rotate(img, angle) for img in edges]
            M_edges = [TF.rotate(img, angle) for img in M_edges]
        ...
        if self.with_random_crop and random.random() > 0:
            ...
            edges = [TF.resized_crop(img, i, j, h, w,
                                     size=(self.img_size, self.img_size),
                                     interpolation=Image.CUBIC)
                     for img in edges]

            M_edges = [TF.resized_crop(img, i, j, h, w,
                                       size=(self.img_size, self.img_size),
                                       interpolation=Image.CUBIC)
                       for img in M_edges]
        ...
        if self.with_scale_random_crop:
            ...
            edges = [pil_rescale(img, target_scale, order=3) for img in edges]
            M_edges = [pil_rescale(img, target_scale, order=3) for img in M_edges]
            ...
            imgs = [pil_crop(img, box, cropsize=self.img_size, default_value=0)
                      for img in imgs]
            edges = [pil_crop(img, box, cropsize=self.img_size, default_value=255)
                     for img in edges]
            M_edges = [pil_crop(img, box, cropsize=self.img_size, default_value=255)
                      for img in M_edges]
        ...
        if to_tensor:
            # to tensor
            imgs = [TF.to_tensor(img) for img in imgs]
            edges = [TF.to_tensor(img) for img in edges]
            M_edges = [TF.to_tensor(img) for img in M_edges]
            ...
            imgs = [TF.normalize(img, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
                      for img in imgs]
            edges = [TF.normalize(img, mean=[0.5], std=[0.5])
                     for img in edges]
            M_edges = [TF.normalize(img, mean=[0.5], std=[0.5])
                       for img in M_edges]

        return imgs, edges, M_edges, labels, labels_A, labels_B

    def transform_muti_scale(self, imgs_1, imgs_2, edges, M_edges, labels, labels_A, labels_B, to_tensor=True):
        """
        :param to_tensor: bool
        ...
        return imgs_1, imgs_2, edges, M_edges, labels, labels_A, labels_B
# ---------------------------------------------------------------------------------
