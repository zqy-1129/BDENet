...
class FC_Siam_Conv(nn.Module):
    def __init__(self, num_classes):
        super(FC_Siam_Conv, self).__init__()
        self.backbone = SiamUnet_conc(3, num_classes)
        self.CD = SiamUnet_conc(num_classes, 2)

    def forward(self, x, y):
        Seg_A = self.backbone(x, y)
        Seg_B = self.backbone(y, x)
        CD = self.CD(Seg_A, Seg_B)
        return Seg_A, Seg_B, CD


if __name__ == '__main__':
    model = FC_Siam_Conv(6)
