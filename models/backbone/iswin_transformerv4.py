...
class ConvAttnBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, act_layer=nn.ReLU, groups=1,
        ...
        self.conv__1 = nn.Conv2d(in_channels, med_planes, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x, H, W):
        ...
        x = x.permute(0, 2, 1)
        x = x.view(B, C, H, W)
        x = self.act1(self.bn1(self.conv__1(x)))
...
class ContextFuseBlock(nn.Module):
    ...
    def forward(self, x, y):
        ...
        atten_x, atten_y = self.attn(self.norm1(x), H_x, W_x), self.attn(self.norm2(y), H_y, W_y)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # self.lk = nn.Linear(conv_channel, dim, bias=False)
        depths = [3, 4, 6, 3]
        sr_ratios = [8, 4, 2, 1]
        ...
        # self.fuseblock = Block(
        #         dim=dim, num_patches=8, num_heads=num_heads, mlp_ratio=mlp_ratio,
        #         qkv_bias=qkv_bias,
        #         qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, drop_path=drop_path[depth-1] if isinstance(drop_path, list) else drop_path,
        #         norm_layer=norm_layer, sr_ratio=sr_ratios[stage],
